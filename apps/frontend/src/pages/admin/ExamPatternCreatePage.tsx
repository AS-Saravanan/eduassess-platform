/**
 * ExamPatternCreatePage – /admin/exam-patterns/create
 */
...
export const ExamPatternCreatePage: React.FC = () => {
  const navigate = useNavigate();
  const [isMobileSidebarOpen, setIsMobileSidebarOpen] = useState(false);
  const [isLoggingOut, setIsLoggingOut] = useState(false);

  const { createPattern } = useExamPatternContext();
        ...
        <header className="bg-white border-b border-slate-100 h-16 px-6 sm:px-8 flex items-center justify-between sticky top-0 z-30 shadow-xs">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:block">
              ...
              <p className="text-sm font-bold text-slate-900 mt-0.5">Create New Pattern</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
        ...
        <main className="flex-1 py-8 px-6 sm:px-8 lg:px-10 overflow-y-auto max-w-4xl w-full mx-auto">
          <div className="mb-6">
            ...
            <p className="text-sm text-slate-500 font-medium mt-1">
              Define the structure, sections, and reference materials for a new assessment.
            </p>
          </div>
          <ExamPatternForm
            onSubmit={handleSubmit}
            onCancel={() => navigate("/admin/exam-patterns")}
            isEditMode={false}
          />
        </main>
      </div>
...
export default ExamPatternCreatePage;
