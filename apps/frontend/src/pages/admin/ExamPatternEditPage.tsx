/**
 * ExamPatternEditPage – /admin/exam-patterns/:id/edit
 */
...
import { ExamPatternForm } from "../../features/admin/exam-patterns/ExamPatternForm";
import { useExamPatternContext } from "../../features/admin/exam-patterns/ExamPatternContext";
import type { ExamPatternDraft } from "../../features/admin/exam-patterns/examPattern.types";

export const ExamPatternEditPage: React.FC = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const [isMobileSidebarOpen, setIsMobileSidebarOpen] = useState(false);
  const [isLoggingOut, setIsLoggingOut] = useState(false);

  const { getById, updatePattern } = useExamPatternContext();
  const pattern = id ? getById(id) : undefined;
  ...
  const handleSubmit = (draft: ExamPatternDraft) => {
    if (!id) return;
    updatePattern(id, draft);
    navigate(`/admin/exam-patterns/${id}`);
  };

  // Derive draft from existing pattern (omit id + computed totalMarks)
  const initialData: ExamPatternDraft | undefined = pattern
    ? {
        name: pattern.name,
        grade: pattern.grade,
        ...
        samplePapers: pattern.samplePapers,
      }
    : undefined;
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:block">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest">
                Admin Portal · Exam Patterns
              </p>
              <p className="text-sm font-bold text-slate-900 mt-0.5">Edit Pattern</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
        ...
        <main className="flex-1 py-8 px-6 sm:px-8 lg:px-10 overflow-y-auto max-w-4xl w-full mx-auto">
          {!pattern ? (
            <div className="text-center py-24 space-y-3">
              <p className="text-lg font-bold text-slate-700">Exam pattern not found</p>
              <button
                onClick={() => navigate("/admin/exam-patterns")}
            ...
            <>
              <div className="mb-6">
                <h1 className="text-xl font-display font-extrabold text-slate-950">
                  Edit Exam Pattern
                </h1>
                <p className="text-sm text-slate-500 font-medium mt-1">
                  Update the structure and details for <span className="font-bold text-slate-700">{pattern.name}</span>.
                </p>
              </div>
              <ExamPatternForm
                initialData={initialData}
                onSubmit={handleSubmit}
                onCancel={() => navigate(`/admin/exam-patterns/${id}`)}
                isEditMode
              />
            </>
          )}
...
export default ExamPatternEditPage;
