/**
 * ExamPatternsPage – /admin/exam-patterns
 *
 * Primary list page. Owns the ExamPatternProvider (feature context root)
 * so all child routes share the same pattern state.
 */
...
import { ExamPatternList } from "../../features/admin/exam-patterns/ExamPatternList";
...
export const ExamPatternsPage: React.FC = () => {
  ...
  const { patterns } = useExamPatternContext();
    ...
    <div className="min-h-screen bg-slate-50/60 text-slate-900 font-sans flex flex-col md:flex-row relative">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-white border-b border-slate-100 h-16 px-6 sm:px-8 flex items-center justify-between sticky top-0 z-30 shadow-xs">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:block">
              ...
              <p className="text-sm font-bold text-slate-900 mt-0.5">Exam Patterns</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
        ...
        <main className="flex-1 py-8 px-6 sm:px-8 lg:px-10 overflow-y-auto max-w-7xl w-full mx-auto">
          <ExamPatternList patterns={patterns} />
        </main>
      </div>
    </div>
  );
