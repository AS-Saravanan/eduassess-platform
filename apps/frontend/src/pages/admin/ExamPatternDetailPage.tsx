/**
 * ExamPatternDetailPage – /admin/exam-patterns/:id
 */
...
import { ExamPatternDetails } from "../../features/admin/exam-patterns/ExamPatternDetails";
import { useExamPatternContext } from "../../features/admin/exam-patterns/ExamPatternContext";

export const ExamPatternDetailPage: React.FC = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const [isMobileSidebarOpen, setIsMobileSidebarOpen] = useState(false);
  const [isLoggingOut, setIsLoggingOut] = useState(false);

  const { getById } = useExamPatternContext();
  ...
  return (
    <div className="min-h-screen bg-slate-50/60 text-slate-900 font-sans flex flex-col md:flex-row relative">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-slate-100 h-16 px-6 sm:px-8 flex items-center justify-between sticky top-0 z-30 shadow-xs">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:block">
              ...
              <p className="text-sm font-bold text-slate-900 mt-0.5">Pattern Details</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
        ...
        <main className="flex-1 py-8 px-6 sm:px-8 lg:px-10 overflow-y-auto max-w-5xl w-full mx-auto">
          {!pattern ? (
            <div className="text-center py-24 space-y-3">
              <p className="text-lg font-bold text-slate-700">Exam pattern not found</p>
              <p className="text-sm text-slate-400">
                This pattern may have been deleted or the URL is incorrect.
              </p>
              <button
            ...
            <ExamPatternDetails
              pattern={pattern}
              onEdit={() => navigate(`/admin/exam-patterns/${id}/edit`)}
              onBack={() => navigate("/admin/exam-patterns")}
            />
          )}
        </main>
      </div>
...
export default ExamPatternDetailPage;
